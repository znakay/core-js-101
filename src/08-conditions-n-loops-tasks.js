...
function isInsideCircle(circle, point) {
  const sizeX = point.x - circle.center.x;
  const sizeY = point.y - circle.center.y;
  return circle.radius ** 2 > sizeX ** 2 + sizeY ** 2;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const firstBracked = isStartIncluded ? '[' : '(';
  const lastBracked = isEndIncluded ? ']' : ')';
  return `${firstBracked + (a > b ? b : a)}, ${(a > b ? a : b) + lastBracked}`;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < newStr.length; i += 1) {
    for (let j = 0; j < config.length; j += 1) {
      if (newStr[i] === config[j][0] && newStr[i + 1] === config[j][1]) {
        newStr.splice(i, 2);
        i = -1;
        break;
      }
